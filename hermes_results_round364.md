...
- **HTTP**: `curl -s -o /dev/null -w "%{http_code}" http://localhost:3005` → `200`
- **Console**: `BASE_URL=http://localhost:3005 npx tsx scripts/browser.ts console http://localhost:3005` → ✅ 0 errors
- **Flow**: `BASE_URL=http://localhost:3005 npx tsx scripts/browser.ts flow` → ✅ 5/5 steps (首页/Diagnosis/Result/Execute/Submit)
- **Mobile**: `BASE_URL=http://localhost:3005 npx tsx scripts/browser.ts mobile` → ✅ 3/3 (首页/Diagnosis/Result)

## WeShop.ai vs easyuse.ai 对比分析（R364: 2026-05-04 19:30）

### WeShop 关键数据（curl提取）
- **模型数量**: Grok(12), Seedance(10), Kling(10), z-image(8), Seedream(7), Wan AI(6), Sora(6), GPT Image 2(5), Veo(4), Nano-Banana(4), Midjourney(4), Hailuo(4), Fire Red(4)
- **用户量**: 3,000,000+ users worldwide
- **NYSE上市**: MOGU (XNYS:MOGU) — 4处NYSE链接
...
{
  "success": true,
  "summary": "R364健康检查全量通过(HTTP200/Console0/Flow5/5/Mobile3/3)。连续稳定79轮，代码级差距0。WeShop无新增变化，所有剩余差距均为用户提供决策级别(A/B/C级)。",
  "output": {
    "修复内容": "无 — 全量通过，代码级差距0",
