...
export async function POST(req: NextRequest) {
  try {
    ...
    const { action, type, leadId } = body as { action: string; type: string; leadId?: string };

    await new Promise((r) => setTimeout(r, 1500));

    const results: Record<string, { title: string; items: string[] }> = {
      copywriting: {
        title: "生成结果：3条引流文案",
        items: [
    ...
    const result = results[action] ?? { title: "生成完成", items: ["结果已生成"] };
